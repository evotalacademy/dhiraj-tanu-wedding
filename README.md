...
previous one, plus a small Back button top-right. If a page's content is
taller than the screen, the first swipes simply scroll it as usual — only
once you reach the bottom (or top, going back) does the next swipe change
the page, so nothing feels cut short. Each transition is a soft cinematic
crossfade-and-lift (scale, blur, a brief light sweep), not a plain slide.
A subtle "swipe up" cue appears near the bottom of each page and fades
once you've swiped for the first time. Three pages (Mehndi & Sangeet,
Haldi, Shubh Vivah) play a short 2–3 second decorative particle effect
each time they're entered. Buttons, links and phone numbers never trigger
page navigation themselves. A small music mute/unmute toggle appears once
the card has been opened; it mutes rather than stops the track, so
playback position is never lost. Visitors on a desktop browser (or with
