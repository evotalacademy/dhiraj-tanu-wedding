(function () {
  ...
  function applyLanguage(lang) {
    ...
    // Family blessing line (opening scene) — three short lines built from
    // config.families, rather than hard-coded, so a name change in
    // config.js is enough.
    var fam = WEDDING_CONFIG.families[lang];
    setText("familyGroom", fam.groomParents);
    setText("familyAndWord", copy.familyAnd);
    setText("familyBride", fam.brideParents);

    // Contact line (final scene) — static text, no form, built from
    // config.contacts.
    (WEDDING_CONFIG.contacts || []).forEach(function (person, i) {
      setText("contactRow" + i, person.name + " \u00B7 " + person.phone);
    });
  }
  ...
  function goTo(index, opts) {
    ...
    if (els.musicToggle) els.musicToggle.hidden = index === 0; // hidden only on the language screen
  ...
  /* Tap-anywhere-to-advance, but never when tapping an interactive control.
     Note: this no longer touches music — playback already started at the
     language-selection tap, and "Tap to Begin" is navigation only. */
  ...
  /* Language selection — this tap is also the user gesture that starts
     the music (required for iOS/Safari autoplay rules). Music must
     start here, immediately, not on the later "Tap to Begin". */
  ...
  /* ------------------------------------------------------------
     MUSIC — starts on the language-selection tap (a real user
     ...
     restarted by scene changes or "Tap to Begin", since it lives
     entirely outside the scene-navigation logic. The control button
     MUTES (audio keeps playing, silently, position keeps advancing)
     rather than pausing — muting never touches playback position.
  ------------------------------------------------------------ */
  ...
  // Robust loop: the `loop` attribute on <audio> handles this on its
  // own almost everywhere, but some mobile browsers have been known to
  // drop it under certain conditions — this listener is a harmless,
  // redundant safety net so the music never simply stops.
  if (els.music) {
  ...
  if (els.musicToggle) {
    els.musicToggle.addEventListener("click", function (e) {
      e.stopPropagation(); // never triggers scene navigation
      if (!els.music) return;
      if (!musicRequested) {
        // Playback never actually started (e.g. autoplay was blocked at
        // language-selection time) — this tap is itself a fresh user
        // gesture, so use it to start playback rather than mute a
        // track that was never playing.
        startMusic();
        return;
      }
      // Mute/unmute only — playback and position are never touched,
      // so the song keeps advancing silently while muted.
  ...
  function init() {
    if (state.lang) {
      ...
      goTo(1);
      // Returning within the same session (e.g. a reload) — there's no
      // fresh tap here for autoplay to ride on, so don't attempt
      // playback yet. The toggle shows an honest "silent" state; the
      // visitor's first tap on it starts playback as a real gesture.
